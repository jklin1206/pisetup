...
export async function runToggleSkillsCommand(ctx: ExtensionCommandContext, deps: ToggleSkillsCommandDeps): Promise<void> {
  ...
  try {
    ...
    ctx.ui.notify(`Skill Toggle failed to scan skills: ${error instanceof Error ? error.message : String(error)}`, "error");
    return;
  }

  if (skills.length === 0) {
    ctx.ui.notify("Skill Toggle: no skills found in user or project skill directories", "info");
    return;
  }
  ...
  try {
    ...
    ctx.ui.notify(`Skill Toggle failed to plan changes: ${error instanceof Error ? error.message : String(error)}`, "error");
    return;
  }

  if (changes.length === 0) {
    ctx.ui.notify("Skill Toggle: no changes to apply", "info");
    return;
  }
...
function formatApplyResult(result: ApplyResult): string {
  const lines = [`Skill Toggle applied ${result.applied.length} change${result.applied.length === 1 ? "" : "s"}.`];
